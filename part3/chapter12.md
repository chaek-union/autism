...
유전되는 희귀 변이의 기여를 이해하는 것은 자폐의 유전적 구조를 완성하는 데 필수적이다. 신생변이가 단발성 자폐의 약 10~30%를 설명한다면, 유전된 희귀 변이는 특히 다발성 가족에서 추가적인 위험을 설명한다. 하지만 이 두 종류의 희귀 변이를 합쳐도 자폐의 유전적 위험 전체를 설명하지는 못한다. 나머지는 어디에 있는가? 인구에서 흔하게 관찰되는 일반 변이 수천 개가 각각 아주 작은 효과를 합산하여 자폐 위험의 가장 큰 부분을 차지하고 있다.
