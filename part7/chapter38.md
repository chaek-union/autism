...
CHD8, ADNP, ARID1B라는 세 유전자는 각각 다른 크로마틴 복합체에 속하지만, 공통적으로 뇌 발달 초기의 유전자 발현 프로그램을 조절한다. 이 유전자들의 변이가 시냅스 유전자의 변이와 궁극적으로 유사한 표현형(자폐스펙트럼장애)으로 이어지는 것은, 크로마틴 조절이 시냅스 유전자의 발현을 상위에서 통제하기 때문이다. 지금까지 살펴본 유전자들에서 변이는 대부분 기능의 상실을 의미했다. 하지만 같은 유전자의 변이가 기능을 상실시키느냐 강화하느냐에 따라 전혀 다른 질환이 나타날 수 있다면 어떻게 될까?
