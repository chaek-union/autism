...
이 다섯 전환의 결과로 현재의 연구 규모가 만들어졌다. Satterstrom et al. (2020) 연구에서 35,584명, Fu et al. (2022) 연구에서 63,237명, Kim et al. (2025) 연구에서 78,685명의 통합 분석이 가능해졌다. 동시에 한 가지 역설도 드러났다. 유전체 데이터가 가장 풍부한 코호트(SSC, SPARK, MSSNG)에는 성인기 결과 데이터가 거의 없고, 성인기 결과를 장기 추적하는 코호트(UK Biobank, 덴마크 등록 체계)에는 심층 유전체 데이터가 부족하다. 유전적 원인을 아는 아이가 성인이 되었을 때 어떤 삶을 사는지를 알려면, 이 두 종류의 데이터가 한 코호트 안에서 만나야 한다. 자폐 유전학의 다음 과제 중 하나가 여기에 있다.
