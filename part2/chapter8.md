...
자폐 시퀀싱 컨소시엄(Autism Sequencing Consortium, ASC)은 이 필요에 응답하여 만들어진 국제 협력체다 (Buxbaum et al. 2012). 마운트 시나이 의과대학, 브로드 연구소, 예일 대학, 피츠버그 대학 등 주요 연구 기관의 유전학자, 통계학자, 임상가들이 참여하여 엑솜 시퀀싱 데이터를 공유하고 통합 분석하는 프레임워크를 구축했다.

ASC가 만들어진 배경에는 2012년의 경험이 있다. 그해에 네 개의 독립적인 연구팀이 거의 동시에 SSC 가족들의 엑솜 시퀀싱 결과를 발표하여 신생 코딩 변이가 자폐의 중요한 원인이라는 것을 확립했다(11장에서 상세히 다룬다). 각 팀이 독립적으로 같은 결론에 도달한 것은 발견의 신뢰성을 높여주었지만, 과학자들은 거기서 멈추지 않았다. 개별 팀이 각자의 데이터로 분석하는 대신, 모든 데이터를 하나로 합쳐서 분석하면 훨씬 더 많은 유전자를 발견할 수 있다는 것을 알았기 때문이다. ASC는 이 인식에서 출발하여, 자신이 찾은 결과를 솔직하게 공유하고, 상대방의 분석에 개입하여 함께 당대의 데이터에서 최적의 결과를 도출하려는 협업 문화를 만들어냈다.

현대 과학은 한 명의 천재가 혼자 해내는 것이 아니라, 많은 연구자가 서로 논의하고, 지적하고, 보완하면서 이루어진다. ASC의 정기 화상 회의에는 세계 각국의 연구자들이 참여했다. 누군가에게는 새벽이고, 누군가에게는 그 나라의 공휴일이었다. 시차와 국경을 넘어 과학자들이 이렇게 모인 것은, 자폐에 대해 더 정확히 이해하고자 하는 공통된 열망 때문이었다. 이 책의 저자도 ASC의 일원으로 참여하면서 이 협업의 과정을 직접 경험했다. ASC의 핵심 전략은 여러 코호트의 데이터를 하나로 합쳐서 분석하는 것(데이터 풀링, data pooling)이었다. SSC, AGRE, 그리고 다수의 개별 코호트에서 수행된 엑솜 시퀀싱 결과를 하나의 분석 파이프라인 안에서 함께 분석함으로써, 개별 코호트에서는 불가능했던 통계적 검정력을 확보하는 것이었다.
